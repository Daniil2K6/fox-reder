...
class Book(Base):
     ...
     metadata_translations = relationship("BookMetadataTranslation", back_populates="book", cascade="all, delete-orphan")
...
class Translation(Base):
     """Кеш переводов абзацев текста книги"""
     __tablename__ = "translations"

     id = Column(Integer, primary_key=True, index=True)
     book_id = Column(Integer, ForeignKey("books.id", ondelete="CASCADE"), nullable=False, index=True)
     paragraph_id = Column(String(64), nullable=False)  # ID абзаца (ch-0-p-5, etc)
     original_text = Column(Text, nullable=False)
     source_language = Column(String(16), nullable=False)  # Язык оригинала (en, ru, etc)
     target_language = Column(String(16), nullable=False, index=True)  # Язык перевода
     translated_text = Column(Text, nullable=False)
     translator = Column(String(32), default="google", nullable=False)  # "google" или "local"
     quality_score = Column(Integer, default=100)  # 0-100, для выбора лучшего переводчика
     created_at = Column(DateTime, default=datetime.utcnow)

     book = relationship("Book", back_populates="translations")

     __table_args__ = (
         UniqueConstraint("book_id", "paragraph_id", "target_language", name="uq_translation"),
     )


class BookMetadataTranslation(Base):
     """Переводы метаданных книги (название, описание, жанры)"""
     __tablename__ = "book_metadata_translations"

     id = Column(Integer, primary_key=True, index=True)
     book_id = Column(Integer, ForeignKey("books.id", ondelete="CASCADE"), nullable=False, index=True)
     language = Column(String(16), nullable=False, index=True)  # Язык перевода
     title = Column(String(512), nullable=True)  # Переведённое название
     description = Column(Text, nullable=True)  # Переведённое описание
     genres = Column(String(512), nullable=True)  # Переведённые жанры (запятой разделённые)
     translator = Column(String(32), default="google", nullable=False)
     created_at = Column(DateTime, default=datetime.utcnow)

     book = relationship("Book", back_populates="metadata_translations")

     __table_args__ = (
         UniqueConstraint("book_id", "language", name="uq_book_metadata_lang"),
     )
